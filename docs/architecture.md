...
├── MDXContent             (Runtime MDX renderer with ReferenceProvider)
...
│   ├── ReferenceCard     (Block-level reference card)
│   ├── ReferenceHover    (Inline hover citation with [^key] syntax)
│   ├── ReferenceList    (Auto-collected reference list)
