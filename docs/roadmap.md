...
| 🔴 高优先 | **Category 详情页** | 拆分为 3 页：Code Practice（tech，标签分组二级列表）、Book Insight（book，占位）、Life Journal（life，占位）。分组配置外置 `config/category-groups.ts` | `app/(blog)/category/[id]/page.tsx`, `config/category-groups.ts` |
